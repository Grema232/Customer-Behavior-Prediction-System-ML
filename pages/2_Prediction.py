...
if st.button("Run Prediction"):
    ...
    m1.metric(
        ...
        f"{prob_purchase*100:.2f}%"
    )

    m2.metric(
        "Probability of No Purchase",
        f"{prob_not_purchase*100:.2f}%"
    )
    ...
    if prob_purchase >= 0.80:
        st.info(
            "High confidence prediction. Strong behavioral purchase signals detected."
        )

    elif prob_purchase >= 0.60:
        st.info(
            "Moderate confidence prediction. Customer shows meaningful purchase intent."
        )

    elif prob_purchase >= 0.40:
        st.info(
            "Uncertain prediction. Behavioral signals are mixed."
        )

    else:
        st.info(
            "Low confidence prediction. Weak purchase intent indicators."
        )
    ...
    st.write(
        f"Model confidence level: **{prob_purchase*100:.2f}%** probability of purchase."
    )
